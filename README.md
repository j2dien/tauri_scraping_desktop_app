...
- **Komentar Instagram**: Jika query GraphQL statis berubah atau mengembalikan respons non-JSON, aplikasi beralih ke respons terstruktur yang digunakan halaman post. Kegagalan satu sumber tidak langsung menghentikan pemrosesan post berikutnya.
- **Ekspor Instagram**: Sheet `Detail Komentar` berisi seluruh komentar yang berhasil diambil dengan sembilan kolom: username, teks komentar, status like post, like komentar, tanggal komentar, URL post, like post, tanggal post, dan caption lengkap.
- **Rate limit dan challenge**: Aplikasi menghentikan pekerjaan pada indikasi autentikasi gagal/pembatasan, menonaktifkan sumber yang gagal berulang, dan membatasi pemeriksaan liker maksimal 20 post per pekerjaan. Pengambilan komentar dibatasi 100 komentar per post dan hasil parsial diberi peringatan. Jangan menjalankan ulang secara bertubi-tubi, memakai rotasi akun/proxy, atau mencoba melewati checkpoint Instagram.
